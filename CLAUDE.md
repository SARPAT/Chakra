...
- **Commit after every code change** — even small additions, minor edits, or single-line fixes must be committed immediately.
- Create frequent, small, and meaningful commits.
- Commit even minor changes if they represent a logical step forward.
- Each commit must reflect a single focused change (avoid large, mixed changes).
